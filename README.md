...
- 우표
  - 우표를 굳이 안 붙여도 편지를 보낼 수는 있지만 약 10일 정도 걸림
- 약
...
- 큰 가방(또는 캐리어)
  - 퇴소 시 짐이 생각보다 많음(생활화, 전투화, 전투복, 베레모 등등)
  - 퇴소 전 쇼핑백(부직포 재질) 구매 기회를 주기 때문에 반드시 가져가진 않아도 됨
  - 쇼핑백 구매 기회는 딱히 안내를 해주지는 않지만 px 2번째 가는날이므로 필요하다면 이때 반드시 사야 함
- 휴지
...
- 고속버스 이용 시
  ...
  - 연무대고속버스터미널에서 육군훈련소까지 마을버스를 타고 간다면 "훈련소앞" 정류장이 아닌 "훈련소입소대대" 정류장에서 내려야한다.
